...
class WordPressExporter {
  ...
  createFrontMatter(post) {
    ...
    const featuredMediaUrl = post._embedded?.['wp:featuredmedia']?.[0]?.href || null;
  ...
  async convertPostToMarkdown(post) {
    ...
    let processedContent = await this.processImages(post.content.rendered, post.slug);
    for (const plugin of this.loadedPlugins) {
      processedContent = plugin.processPostContent(processedContent);
    }
...
console.log(chalk.yellow('------------------'));
console.log(`WordPress URL: ${options.url}`);
console.log(`Output directory: ${options.output}`);
console.log(`Post limit: ${options.limit}`);
console.log(`Code block classes: ${options.codeClasses}`);
console.log(`Custom post type: ${options.customPostType}`);
console.log(`Preserve tags: ${options.preserveTags}`);
